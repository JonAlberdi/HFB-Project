...
class ModbusTcpServer(socketserver.ThreadingTCPServer):
    ...
    def __init__(self, context, framer=None, identity=None,
                 address=None, response=None, handler=None, allow_reuse_address=False,
                 **kwargs):
...
class MyRequestHandler(ModbusConnectedRequestHandler):
    def setup(self):
        ...
        self.server.connected_clients.append(self.client_address)
        _logger.debug("self.client_address [%s:%s]" % self.client_address)

        _logger.debug("self.client_address %s" % self.client_address[0])
       ...
       # _logger.debug("Query Server response %s" % response)
        ...
        if message != self.server.response:
            print("Error en autenticacion")
            self.server.server_close()
            


class MyModbusTcpServer(ModbusTcpServer):
    def __init__(self, *args, **kwargs):
        super(MyModbusTcpServer, self).__init__(*args, **kwargs)
...
# Creation Factories
# --------------------------------------------------------------------------- #
def StartTcpServer(context=None, identity=None, address=None, response=None,
    ...
    server = MyModbusTcpServer(
        context, framer, identity, address, response, handler=MyRequestHandler, **kwargs)
